%{
% FILENAME: procrustes_Gabor_resps_decoding_basic.m
% AUTHOR:   Zitong Wang
% DATE:     2025-07-13
%
% DESCRIPTION:
%   Performs Procrustes-based transfer decoding on synthetic trial data
%   generated from Gabor filter models. This script is designed to run on
%   two specific models: a 'complex' cell model and a combined 'simple' cell
%   model ('even and odd combined'). It loads the corresponding trial data,
%   runs the decoding analysis for all stimulus pairs, and saves the results.
%}
...
% Load the synthetic Gabor data, prepare save directories, and format the
% data for the decoding functions.
fprintf('Loading data for "%s" model...\n', filter_name);

% --- Load synthetic trial data ---
spike_data = load(strcat('..\..\Gabor_filters_response_data\sampled_trials\', filter_name, '\', filter_name, '.mat'), 'response_array');
spike_data = spike_data.response_array;

% --- Prepare output directory ---
save_path = ['..\..\results\tables\', filter_name, '\'];
...
% This section allows for flexible selection of the number of units to use
% in the decoding analysis.
use_all_neurons = true; % SET TO 'true' to use all available units, or 'false' to use a subset.

subset_neuron_num = 48; % Number of units to use if not using all (e.g., to match complex filters).
all_neuron_num = size(data_trial{1}, 2);

if use_all_neurons
    sample_neuron_num = all_neuron_num;
    fprintf('Using all %d available units.\n', all_neuron_num);
else
    sample_neuron_num = subset_neuron_num;
    fprintf('Using a subset of %d units.\n', subset_neuron_num);
end
...
% This section runs the Procrustes decoding for all six ordered stimulus
% pairs and saves the results for each pair immediately after completion.

% --- Set repetition and randomization parameters ---
neuron_sample_repeat = 50; % Number of times to resample neuron subsets
trial_sample_repeat = 15;  % Number of times to resample trial partitions
rng(1); % Set the random seed for reproducibility

tic;
fprintf('Starting decoding analysis for "%s" model...\n', filter_name);

% --- Run decoding for each stimulus pair ---
acec_results = Gabor_decoding('ac','ec',data_trial,labels,sample_neuron_num,neuron_sample_repeat,trial_sample_repeat);
save([save_path,'acec_results.mat'],'acec_results');
fprintf('Saved acec_results.mat\n');

ecex_results = Gabor_decoding('ec','ex',data_trial,labels,sample_neuron_num,neuron_sample_repeat,trial_sample_repeat);
save([save_path,'ecex_results.mat'],'ecex_results');
fprintf('Saved ecex_results.mat\n');

acex_results = Gabor_decoding('ac','ex',data_trial,labels,sample_neuron_num,neuron_sample_repeat,trial_sample_repeat);
save([save_path,'acex_results.mat'],'acex_results');
fprintf('Saved acex_results.mat\n');

ecac_results = Gabor_decoding('ec','ac',data_trial,labels,sample_neuron_num,neuron_sample_repeat,trial_sample_repeat);
save([save_path,'ecac_results.mat'],'ecac_results');
fprintf('Saved ecac_results.mat\n');

exec_results = Gabor_decoding('ex','ec',data_trial,labels,sample_neuron_num,neuron_sample_repeat,trial_sample_repeat);
save([save_path,'exec_results.mat'],'exec_results');
fprintf('Saved exec_results.mat\n');

exac_results = Gabor_decoding('ex','ac',data_trial,labels,sample_neuron_num,neuron_sample_repeat,trial_sample_repeat);
...
%%
function result_one_cell = Gabor_decoding(stim1,stim2,data_trial,labels,sample_neuron_num,neuron_sample_repeat,trial_sample_repeat)
% stim1 is transformed stim2 is the training data

pair_wise_data_trial = pair_pcaloader(data_trial,stim1,stim2);
total_neuron = size(pair_wise_data_trial,2);
...
parfor neuron_repeat = 1:neuron_sample_repeat
    ...
    sampled_neuron_idx = randperm(total_neuron,sample_neuron_num);
