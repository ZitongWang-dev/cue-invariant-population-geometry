%{
Filename: procrustes_decoding_neuron_shuffled.m
Author: Zitong Wang
Date: 2025-06-24

Description:
    Archive Procrustes-based transfer decoding on neuronal datasets from two monkeys (FR, KO),
    applied independently to each visual area (V1 or V2). This version applies neuron-index
    shuffling in the test data to evaluate how randomizing the order of sampled neurons affects
    transfer decoding performance. It loads trial-by-neuron firing arrays for three stimulus
    conditions, executes both standard and shuffled Procrustes alignments, and calculates
    transfer decoding accuracies under full, partial, and randomized transformations.

Inputs:
    - neuronal_data/<monkey>/<area>/<monkey>_<area>_allstim.mat
        Contains variable 'three_stim_array': cell array of [trial x neuron] matrices per stimulus.
    - timewindow: two-element vector [start_ms end_ms] defining the spike-count window.

Outputs:
    - MAT-files saved to:
      results/decoding_outputs/procrustes_decoding_neuron_shuffled_results/<monkey>/<area>/
      Files: acec_results.mat, ecex_results.mat, acex_results.mat, ecac_results.mat,
             exec_results.mat, exac_results.mat
%}
...
save_path = fullfile('..','..','results','decoding_outputs','procrustes_decoding_neuron_shuffled_results', monkey, vp);
...
%% Neuron sampling setup
% Define how many neurons to sample for each monkey-area combination
%   {FR-V1, FR-V2, KO-V1, KO-V2}
...
%% Decoding parameters
neuron_sample_repeat = 50;   % number of random neuron subsets
trial_sample_repeat = 15;    % number of trial splits per subset
rng(1);                      % reproducible RNG state

tic
acec_results = procrustes_decoding_shuffle('ac','ec',data_trial,labels,neuron_num_list,neuron_sample_repeat,trial_sample_repeat);
toc
save([save_path,'acec_results.mat'],'acec_results')

ecex_results = procrustes_decoding_shuffle('ec','ex',data_trial,labels,neuron_num_list,neuron_sample_repeat,trial_sample_repeat);
toc
save([save_path,'ecex_results.mat'],'ecex_results')

acex_results = procrustes_decoding_shuffle('ac','ex',data_trial,labels,neuron_num_list,neuron_sample_repeat,trial_sample_repeat);
toc
save([save_path,'acex_results.mat'],'acex_results')

ecac_results = procrustes_decoding_shuffle('ec','ac',data_trial,labels,neuron_num_list,neuron_sample_repeat,trial_sample_repeat);
toc
save([save_path,'ecac_results.mat'],'ecac_results')

exec_results = procrustes_decoding_shuffle('ex','ec',data_trial,labels,neuron_num_list,neuron_sample_repeat,trial_sample_repeat);
toc
save([save_path,'exec_results.mat'],'exec_results')

exac_results = procrustes_decoding_shuffle('ex','ac',data_trial,labels,neuron_num_list,neuron_sample_repeat,trial_sample_repeat);
save([save_path,'exac_results.mat'],'exac_results')
toc
%%
function results = procrustes_decoding_shuffle(stim1,stim2,data_trial,labels,neuron_num_list,neuron_sample_repeat,trial_sample_repeat)
...
for neuron_squence = 1
    ...
    parfor neuron_repeat = 1:neuron_sample_repeat
        ...
        % shuffle the neuron sequence of test data, stim1
        shuffled_sampled_neuron_idx = sampled_neuron_idx(randperm(length(sampled_neuron_idx)));
        stim1_data_sample = stim1_data(:,shuffled_sampled_neuron_idx);
