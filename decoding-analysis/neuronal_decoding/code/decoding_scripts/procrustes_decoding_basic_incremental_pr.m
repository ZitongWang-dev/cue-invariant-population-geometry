...
Description:
    ...
    Extends procrustes_decoding_basic_incremental.m in three ways:
      (1) Neuron-count sweep uses uniform 10-unit steps (10,20,...,max) instead
          of powers of two, so population sizes match the Gabor sweep exactly.
      (2) At each sampled neuron subset, computes the participation ratio of the
          trial-averaged signal manifold for BOTH conditions in the pair, tagged
          by condition, to quantify intrinsic dimensionality alongside decoding.
      (3) Output is reorganized as a struct per neuron-count for clean pooling.
...
Outputs:
    - MAT-files saved to:
      results/decoding_outputs/procrustes_decoding_basic_incremental_pr_results/<monkey>/<area>/
...
save_path = fullfile('..','..','results','decoding_outputs','procrustes_decoding_basic_incremental_pr_results', monkey, vp);
