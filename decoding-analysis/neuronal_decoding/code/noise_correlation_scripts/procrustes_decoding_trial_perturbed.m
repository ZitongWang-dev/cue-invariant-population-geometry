%{
Filename: procrustes_decoding_trial_perturbed.m
Author: Zitong Wang
Date: 2026-04-21

Description:
    Variant of procrustes_decoding_basic.m that applies a per-neuron,
    per-stimulus trial perturbation (random shuffle or deterministic affine
    shift) immediately after loading trial data, and before any downstream
    processing. Used to test how much of the Procrustes transfer-decoding
    performance depends on preserved noise-correlation structure in the
    pseudopopulation.

    Two perturbation modes are supported (see apply_trial_perturbation.m):
      'none'    - pass-through baseline (identical to procrustes_decoding_basic)
      'shuffle' - independent random permutation of 10 trials per neuron
                  per stimulus (matches the random-shuffle baseline in
                  noise_correlation_analysis.m)
    ...
    All downstream steps (zscore, Procrustes alignment, SVM training,
    partial-transform controls, random sampling over neurons and trials)
    match the original procrustes_decoding_basic.m byte-for-byte.
...
Outputs:
    - MAT-files saved to:
      results/decoding_outputs/procrustes_decoding_trial_perturbed_results/<perturb_mode>/<monkey>/<area>/
...
save_path = fullfile('..','..','results','decoding_outputs', ...
    'procrustes_decoding_trial_perturbed_results', perturb_mode, monkey, vp);
...
if ~strcmp(perturb_mode, 'none')
    rng(perturb_seed);   % reproducible shuffles; no-op for deterministic affine
    for i = 1:num_conditions
        data_trial{i} = apply_trial_perturbation( ...
            data_trial{i}, label_trial{i}, perturb_mode, perturb_opts);
    end
    fprintf('Trial perturbation (%s) applied to all %d conditions.\n', ...
        perturb_mode, num_conditions);
end
...
%% Run Procrustes decoding for all stimulus pairs
tic;
