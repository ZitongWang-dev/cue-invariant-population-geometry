...
fig_prefix = sprintf('nc_%s_%s_nan%s', monkey, vp, nan_mode);
    ...
    if run_shuffle
        ...
        for sh = 1:n_shuffles
            ...
            [nc_shuf, ~] = compute_nc_geomean( ...
                data_by_stim_shuf, mean_rates_floored, pair_idx, N_pairs, N_stimuli, nan_mode);
            ...
            % Mean and variance of NC across stimuli (NaN-aware)
            shuf_mean_nc(:, sh) = mean(nc_shuf, 2, 'omitnan');
            shuf_var_nc(:, sh)  = var(nc_shuf, 0, 2, 'omitnan');

            % Population-level summary statistics (NaN-aware)
            shuf_pop_mean_nc(sh)           = mean(shuf_mean_nc(:, sh), 'omitnan');
            shuf_pop_median_geomean_nc(sh) = median(shuf_geomean_nc_corr(:, sh), 'omitnan');
        ...
        % Real summary stats for comparison (NaN-aware)
...
% Colors: same-session = dark, cross-session = light
...
for r = 1:N_renderings
    ...
    text(0.02, 0.79, sprintf('Shuffle mean: %.3f', nmean(shuf_vals)), ...
        'Units', 'normalized', 'FontSize', 9, 'Color', [0.3 0.3 0.3], ...
        'VerticalAlignment', 'top', 'FontWeight', 'bold');
    text(0.02, 0.71, sprintf('Shuffle median: %.3f', nmedian(shuf_vals)), ...
        'Units', 'normalized', 'FontSize', 9, 'Color', [0.3 0.3 0.3], ...
        'VerticalAlignment', 'top');
...
%% FIGURE 7a-c version1: Stage 2 Scatterhist Original vs Pooled Shuffle (one per rendering)
for r = 1:N_renderings
    rend = renderings{r};
    clr = rendering_colors.(rend);
    ...
    text(h(2), 0.02, 0.80, sprintf('Orig median: %.3f', nmedian(mn_orig)), ...
        'Units', 'normalized', 'FontSize', 9, 'Color', clr, ...
        'VerticalAlignment', 'top');
    text(h(2), 0.02, 0.65, sprintf('Shuf mean: %.3f', nmean(mn_shuf_pooled)), ...
        'Units', 'normalized', 'FontSize', 9, 'Color', [0.4 0.4 0.4], ...
        'VerticalAlignment', 'top', 'FontWeight', 'bold');
    text(h(2), 0.02, 0.50, sprintf('Shuf median: %.3f', nmedian(mn_shuf_pooled)), ...
        'Units', 'normalized', 'FontSize', 9, 'Color', [0.4 0.4 0.4], ...
        'VerticalAlignment', 'top');
    ...
    text(h(3), 0.02, 0.80, sprintf('Orig median: %.4f', nmedian(vn_orig)), ...
        'Units', 'normalized', 'FontSize', 9, 'Color', clr, ...
        'VerticalAlignment', 'top');
    text(h(3), 0.02, 0.65, sprintf('Shuf mean: %.4f', nmean(vn_shuf_pooled)), ...
        'Units', 'normalized', 'FontSize', 9, 'Color', [0.4 0.4 0.4], ...
        'VerticalAlignment', 'top', 'FontWeight', 'bold');
    text(h(3), 0.02, 0.50, sprintf('Shuf median: %.4f', nmedian(vn_shuf_pooled)), ...
        'Units', 'normalized', 'FontSize', 9, 'Color', [0.4 0.4 0.4], ...
        'VerticalAlignment', 'top');
...
%% FIGURE 7a-c version2: Stage 2 Scatterhist Original vs Shuffle by session (one per rendering)
for r = 1:N_renderings
    rend = renderings{r};
    clr = rendering_colors.(rend);
    ...
    % Shuffle data split by session (pool all shuffles)
    mn_shuf_all = all_results.(rend).shuf_mean_nc_all; % [N_pairs x n_shuffles]
    vn_shuf_all = all_results.(rend).shuf_var_nc_all;

    mn_shuf_same  = reshape(mn_shuf_all(is_same_session, :), [], 1);
    vn_shuf_same  = reshape(vn_shuf_all(is_same_session, :), [], 1);
    mn_shuf_cross = reshape(mn_shuf_all(~is_same_session, :), [], 1);
    vn_shuf_cross = reshape(vn_shuf_all(~is_same_session, :), [], 1);

    % Concatenate: shuffles first (background), originals on top; cross before same
    all_mn = [mn_shuf_cross; mn_shuf_same; mn_orig_cross; mn_orig_same];
    all_vn = [vn_shuf_cross; vn_shuf_same; vn_orig_cross; vn_orig_same];
    group_labels = [repmat({'Shuf cross'}, length(mn_shuf_cross), 1); ...
                    repmat({'Shuf same'},  length(mn_shuf_same), 1); ...
                    repmat({'Orig cross'}, length(mn_orig_cross), 1); ...
                    repmat({'Orig same'},  length(mn_orig_same), 1)];

    % Colors: gray=shuf cross, black=shuf same, purple=orig cross, rendering=orig same
    color_4group = [0.7 0.7 0.7; ...      % Shuf cross - gray
                    0.0 0.0 0.0; ...       % Shuf same  - black
    ...
    text(h(2), 0.8, 0.82, sprintf('Orig cross: %.3f', nmean(mn_orig_cross)), ...
        ...
        'VerticalAlignment', 'top', 'FontWeight', 'bold');
    text(h(2), 0.8, 0.69, sprintf('Shuf same: %.3f', nmean(mn_shuf_same)), ...
        'Units', 'normalized', 'FontSize', 9, 'Color', [0 0 0], ...
        'VerticalAlignment', 'top');
    text(h(2), 0.8, 0.56, sprintf('Shuf cross: %.3f', nmean(mn_shuf_cross)), ...
        'Units', 'normalized', 'FontSize', 9, 'Color', [0.4 0.4 0.4], ...
        'VerticalAlignment', 'top');
    ...
    text(h(3), 0.02, 0.90, sprintf('Orig cross: %.4f', nmean(vn_orig_cross)), ...
        'Units', 'normalized', 'FontSize', 9, 'Color', session_color_cross, ...
        'VerticalAlignment', 'top', 'FontWeight', 'bold');
    text(h(3), 0.02, 0.85, sprintf('Shuf same: %.4f', nmean(vn_shuf_same)), ...
        'Units', 'normalized', 'FontSize', 9, 'Color', [0 0 0], ...
        'VerticalAlignment', 'top');
    text(h(3), 0.02, 0.8, sprintf('Shuf cross: %.4f', nmean(vn_shuf_cross)), ...
        'Units', 'normalized', 'FontSize', 9, 'Color', [0.4 0.4 0.4], ...
        'VerticalAlignment', 'top');

    xlabel(h(1), 'Mean noise correlation');
    ylabel(h(1), 'Variance of noise correlation');
    title(h(1), sprintf('%s %s %s — Orig vs Shuffle by session (%d same, %d cross)', ...
        monkey, vp, upper(rend), N_same, N_cross));
    legend(h(1), 'Location', 'best');

    saveas(fig7v2, fullfile(save_path_figs, ...
        sprintf('%s_%s_stage2_mean_vs_variance_nc_vs_shuffle_session.png', fig_prefix, rend)));
end
...
%% FIGURE 9: Delta Mean NC (Original - Shuffle) by session type (3 subplots)
fig9 = figure('Position', [100 100 2500 500]);
for r = 1:N_renderings
    rend = renderings{r};
    clr = rendering_colors.(rend);

    % Delta: original mean NC minus each shuffle's mean NC [N_pairs x n_shuffles]
    delta_nc = all_results.(rend).mean_nc - all_results.(rend).shuf_mean_nc_all;

    % Split by session type and flatten
    delta_same  = reshape(delta_nc(is_same_session, :), [], 1);
    delta_cross = reshape(delta_nc(~is_same_session, :), [], 1);
    ...
    xlabel('\Delta Mean NC (original - shuffle)');
    ylabel('Probability');
    title(sprintf('%s', upper(rend)));
...
sgtitle(sprintf('%s %s — \\Delta Mean NC (Original - Shuffle) by session (%d shuffles)', ...
    monkey, vp, n_shuffles), 'FontSize', 12);
