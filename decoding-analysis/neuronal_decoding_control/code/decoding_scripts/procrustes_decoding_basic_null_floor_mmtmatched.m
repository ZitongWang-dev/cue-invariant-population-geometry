%{
Filename: procrustes_decoding_basic_null_floor.m
Author: Zitong Wang
Date: 2026-07-01

Description:
    CONTROL version of the Procrustes transfer-decoding analysis. Instead of
    decoding the real neural geometry, it decodes a SURROGATE that reproduces
    simulation Model 1 (independent random means) on the real data, to measure
    the empirical PT floor at each area's real neuron count.

    Surrogate construction (per cue, independently):
      1. Decompose the real [500 x N] data into per-stimulus means (10-trial
         averages) and trial residuals (data minus its stimulus mean).
      2. Replace the means with per-neuron MOMENT-MATCHED random draws: for each
         neuron, draw 50 new stimulus-means ~ N(mu_i, sd_i^2) where mu_i, sd_i
         are that neuron's across-stimulus mean and SD of the real means. This
         destroys stimulus structure and any cross-cue relationship while
         preserving each neuron's signal scale.
    ...
    Labels/correspondence are kept intact; both cues get INDEPENDENT random
    means. The decode is otherwise byte-identical to procrustes_decoding_basic.

    Interpretation: PT here is the accuracy the pipeline yields with NO shared
    cross-cue structure, at the real N and matched per-neuron statistics. Real
    PT should be compared against THIS floor, not against 1/50. If real PT ~=
    this floor, the real PT is consistent with the overfitting artifact; if real
    PT clearly exceeds it, there is genuine shared structure beyond the floor.

    LIMITATION: per-neuron random means give a FULL-RANK surrogate (PR ~ 34 at
    N=109), matching simulation Model 1 but NOT the real data's PR. This answers
    "is the Model 1 phenomenon present at my real N". A PR-matched floor
    (preserving each cue's real spectrum, destroying cross-cue correspondence
    via a stimulus-space rotation Procrustes cannot undo) is the rigorous
    follow-up.
...
Outputs:
    - MAT-files saved to:
      results/decoding_outputs/procrustes_decoding_basic_null_floor_results/<monkey>/<area>/
...
%% Run Procrustes decoding on the empirical-null surrogate for all pairs
tic;
acec_results = procrustes_decoding_null_floor('ac','ec', data_trial, labels, neuron_num_list, neuron_sample_repeat, trial_sample_repeat);
save(fullfile(save_path, 'acec_results.mat'), 'acec_results');

ecex_results = procrustes_decoding_null_floor('ec','ex', data_trial, labels, neuron_num_list, neuron_sample_repeat, trial_sample_repeat);
save(fullfile(save_path, 'ecex_results.mat'), 'ecex_results');

acex_results = procrustes_decoding_null_floor('ac','ex', data_trial, labels, neuron_num_list, neuron_sample_repeat, trial_sample_repeat);
save(fullfile(save_path, 'acex_results.mat'), 'acex_results');

ecac_results = procrustes_decoding_null_floor('ec','ac', data_trial, labels, neuron_num_list, neuron_sample_repeat, trial_sample_repeat);
save(fullfile(save_path, 'ecac_results.mat'), 'ecac_results');

exec_results = procrustes_decoding_null_floor('ex','ec', data_trial, labels, neuron_num_list, neuron_sample_repeat, trial_sample_repeat);
save(fullfile(save_path, 'exec_results.mat'), 'exec_results');

exac_results = procrustes_decoding_null_floor('ex','ac', data_trial, labels, neuron_num_list, neuron_sample_repeat, trial_sample_repeat);
save(fullfile(save_path, 'exac_results.mat'), 'exac_results');
toc;
%%
function results = procrustes_decoding_null_floor(stim1,stim2,data_trial,labels,neuron_num_list,neuron_sample_repeat,trial_sample_repeat)
% stim1 is transformed(comparison); stim2 is the training data(target)
% CONTROL: both cues are replaced by independent moment-matched surrogates
% (empirical Model 1). Decode is otherwise identical to the basic version.
...
for neuron_squence = 1
    ...
    parfor neuron_repeat = 1:neuron_sample_repeat
        ...
        % --- CONTROL: independent empirical-null surrogates (fresh per repeat) ---
        s1_surr = make_moment_matched_surrogate(s1_raw);
        s2_surr = make_moment_matched_surrogate(s2_raw);
...
function surr = make_moment_matched_surrogate(real_data)
% Empirical Model 1 surrogate for one cue.
% real_data : [500 x n] raw spike counts, blocked by stimulus (10 trials each).
% Returns  : [500 x n] surrogate = per-neuron moment-matched random means
%            + per-neuron resampled real residuals. Destroys stimulus/cross-cue
%            structure; preserves each neuron's signal scale and noise shape.
[T, n] = size(real_data);
n_stim = 50;
...
% moment-matched random means: per neuron, match across-stimulus mean & SD
mu_n = mean(means, 1);                        % [1 x n]
sd_n = std(means, 0, 1);                      % [1 x n]
rand_means = mu_n + sd_n .* randn(n_stim, n); % [50 x n], independent per entry
rand_means_expanded = repelem(rand_means, n_trials, 1);   % [500 x n]

% resample each neuron's residuals with replacement (preserve noise shape)
surr = zeros(T, n);
for i = 1:n
    ridx = randi(T, T, 1);
    surr(:,i) = rand_means_expanded(:,i) + resid(ridx, i);
end
end
