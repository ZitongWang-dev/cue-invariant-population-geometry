%{
% FILENAME: procrustes_ann_decoding_basic.m
% AUTHOR:   Zitong Wang
% DATE:     2025-07-13
%
% DESCRIPTION:
%   Performs Procrustes-based transfer decoding on synthetic trial data
%   generated from ANN (AlexNet/VGG) activations. This script parallels
%   the analysis run on real neuronal data ('procrustes_decoding_basic.m').
%   It loads the ANN data, runs the decoding analysis for all stimulus
%   pairs (AC, EC, EX), and saves the resulting accuracies.
%}
...
layer = '5';     % Pooling layer number as a string
...
% This data is generated by 'generate_ann_trial_data.m'.
...
save_path = fullfile('..', '..', 'results', 'decoding_outputs', 'procrustes_ann_decoding_basic_results', net, ['pool', layer]);
...
% Define the number of units ('neurons') to sample from the ANN layer.
name2neuronidx = struct('alexnet', 96, 'vgg', 64);
sample_neuron_num = name2neuronidx.(net);
...
% pairs. The results for each pair are saved immediately after completion.
tic;
fprintf('Starting decoding analysis...\n');

% --- Run decoding for each stimulus pair ---
acec_results = ann_decoding('ac', 'ec', data_trial, labels, sample_neuron_num, neuron_sample_repeat, trial_sample_repeat);
save(fullfile(save_path, 'acec_results.mat'), 'acec_results');
fprintf('Saved acec_results.mat\n');

ecex_results = ann_decoding('ec', 'ex', data_trial, labels, sample_neuron_num, neuron_sample_repeat, trial_sample_repeat);
save(fullfile(save_path, 'ecex_results.mat'), 'ecex_results');
fprintf('Saved ecex_results.mat\n');

acex_results = ann_decoding('ac', 'ex', data_trial, labels, sample_neuron_num, neuron_sample_repeat, trial_sample_repeat);
save(fullfile(save_path, 'acex_results.mat'), 'acex_results');
fprintf('Saved acex_results.mat\n');

ecac_results = ann_decoding('ec', 'ac', data_trial, labels, sample_neuron_num, neuron_sample_repeat, trial_sample_repeat);
save(fullfile(save_path, 'ecac_results.mat'), 'ecac_results');
fprintf('Saved ecac_results.mat\n');

exec_results = ann_decoding('ex', 'ec', data_trial, labels, sample_neuron_num, neuron_sample_repeat, trial_sample_repeat);
save(fullfile(save_path, 'exec_results.mat'), 'exec_results');
fprintf('Saved exec_results.mat\n');

exac_results = ann_decoding('ex', 'ac', data_trial, labels, sample_neuron_num, neuron_sample_repeat, trial_sample_repeat);
...
%%
function result_one_cell = ann_decoding(stim1,stim2,data_trial,labels,sample_neuron_num,neuron_sample_repeat,trial_sample_repeat)
