%{
Filename: procrustes_decoding_basic.m
Author: Zitong Wang
Date: 2025-06-24

Description:
    Archive Procrustes-based transfer decoding on neuronal datasets from two monkeys (FR, KO),
    applied independently to each visual area (V1 or V2). This script loads trial-by-neuron firing rates for three stimulus conditions, applies Procrustes alignment
    between paired stimulus configurations, and computes transfer decoding accuracies under full, partial, and randomized
    transformations. Randomized sampling across neuron subsets and trial partitions estimates performance variability.
...
Outputs:
    - MAT-files saved to:
      results/decoding_outputs/procrustes_decoding_basic_results/<monkey>/<area>/
...
save_path = fullfile('..','..','results','decoding_outputs','procrustes_decoding_basic_results', monkey, vp);
...
%% Run incremental Procrustes decoding for all stimulus pairs
tic;
