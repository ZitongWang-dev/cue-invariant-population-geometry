...
neurons={'simple','complex'};

csv_col_names=neurons;

out_file_path='.\';
f=fopen([out_file_path  'after_outlier_removal_sig_only_3x3_jitter.csv'],'w');

out_str=['type,ec-ac,ec-ex,ac-ex'];
fprintf(f,'%s\n',out_str);
...
for neuron_group=1:length(neurons)
    ...
    %tuning_data_path=['D:\15386\corr_mat\' monkey '\no_jitter\3_scale_16_orientation\'];
    tuning_data_path=['D:\15386\corr_mat\' monkey '\3x3_jitter_avg_area\3_scale_16_orientation\'];
...
%         title([monkey ' ' neuron ' ' stim_type{1} ' vs ' stim_type{2}])
        ...
        means = [means, mean(ident_diag(~isnan(ident_diag)&curr_sigs_pos))];
        std_errs = [std_errs, std(ident_diag(~isnan(ident_diag)&curr_sigs_pos))/sqrt(length(curr_sigs_pos(curr_sigs_pos>0)))];
        cnts = [cnts, length(curr_sigs_pos(curr_sigs_pos>0))];
    ...
    out_str=[name_to_write '_mean_corr'];
    for i=1:length(cnts)
        out_str=[out_str ',' num2str(means(i))];
    end
    fprintf(f,'%s\n',out_str);
    
    out_str=[name_to_write '_standard_error'];
    for i=1:length(cnts)
        out_str=[out_str ',' num2str(std_errs(i))];
    end
    fprintf(f,'%s\n',out_str);
    
    out_str=[name_to_write '_sig_cnt'];
    for i=1:length(cnts)
...
t_window='40-250';
neurons={'v1','v2'};
csv_col_names={'ko_v1_all','ko_v2_all'};
...
%         title([monkey ' ' neuron ' ' stim_type{1} ' vs ' stim_type{2}])
    ...
    %print(['C:\Users\84705\Desktop\' gabor_type '_' monkey '_pos_sig.eps'],'-depsc','-painters','-loose',gcf)
...
t_window='40-250';
neurons={'v1','v2'};
csv_col_names={'fr_v1_all','fr_v2_all'};
