...
%         upper_bounds_pos_only=[upper_bounds_pos_only;[corr(half_ec_1_ps,half_ec_2_ps),corr(half_ac_1_ps,half_ac_2_ps),corr(half_ex_1_ps,half_ex_2_ps)]];
    ...
    ub_dists=sort(upper_bounds);
    ub_lower_ci=ub_dists(25,:);
    ub_upper_ci=ub_dists(975,:);
    
    std_corrs=std(half_rsam_corrs);
    std_ubs=std(upper_bounds);
    ...
    ub_lower_cis=[ub_lower_cis;ub_lower_ci];
    ub_upper_cis=[ub_upper_cis;ub_upper_ci];
...
ub_lower_cis=ub_lower_cis';
ub_upper_cis=ub_upper_cis';
ub_lower_cis=final_ubs-ub_lower_cis;
ub_upper_cis=ub_upper_cis-final_ubs;
...
for pos = [1,2]%1:size(means,2)
    ...
    errorbar(xpos,bars_to_plot(:,pos),ub_lower_cis(:,pos),ub_upper_cis(:,pos),'LineStyle','none','Color','black')
end
