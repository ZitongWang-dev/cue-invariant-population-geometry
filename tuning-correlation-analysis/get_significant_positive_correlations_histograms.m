...
%         chosen_layer=1:num_neurons;
        ...
        [max_corr,max_corr_neuron] = max(ident_diag);
        max_corr_neuron
        ...
        ident_diag=diag(tuning_corr,0);
        x_edge=-1:0.05:1;
        histogram(ident_diag,x_edge,'DisplayStyle', 'stairs')
        ...
        curr_pos_sig_mean = mean(ident_diag(~isnan(ident_diag)&curr_sigs_pos));
        mean_cnt = sum((curr_pos_sig_mean-mod(curr_pos_sig_mean,0.05)<=ident_diag) & (ident_diag<curr_pos_sig_mean-mod(curr_pos_sig_mean,0.05)+0.05));
        ...
        ylabel('Counts')
        title([stim_type{1} ' vs ' stim_type{2}])
        
        plot([curr_pos_sig_mean,curr_pos_sig_mean],[0,mean_cnt+1],':');
        text(curr_pos_sig_mean,mean_cnt+2,num2str(mean(ident_diag(~isnan(ident_diag)&curr_sigs_pos))));
    ...
    sgtitle([monkey ' ' neuron])
        
    set(gcf,'position',[0,0,1500,500])
    print([ monkey '_' neuron '_pos_sig_hist.eps'],'-depsc','-painters','-loose',gcf)
    close(f)
