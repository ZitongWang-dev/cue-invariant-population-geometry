...
neuron='v2';
layers={'s','4','d'};
csv_col_names={'superficial','layer 4','deep'};


out_file_path='.\';
f=fopen([out_file_path  monkey '_' t_window '_' neuron '_by_layer.csv'],'w');
...
for layer_group=1:3
    ...
    name_to_write=[neuron '_' csv_col_names{layer_group}];
    
    tuning_data_path='D:\15386\corr_mat\tuning_all\';
    ...
    for cnt=0:2
        ...
        good_neurons_layer={neurons_layer_s,neurons_layer_4,neurons_layer_d};
        ...
        chosen_layer=good_neurons_layer{layer_group};
...
for col=1:length(fraction_values)
    ...
    p_val_stat_test_d_4=min(normcdf(z_values),normcdf(-z_values));
end

for col=1:length(fraction_values)
    p_est=(count_values(1,col)+count_values(3,col))/(n_samples(1)+n_samples(3));
    z_values=(fraction_values(1,:)-fraction_values(3,:))./sqrt(p_est*(1-p_est)*(1/n_samples(1)+1/n_samples(3)));
    p_val_stat_test_d_s=min(normcdf(z_values),normcdf(-z_values));
end

for col=1:length(fraction_values)
    p_est=(count_values(2,col)+count_values(3,col))/(n_samples(2)+n_samples(3));
    z_values=(fraction_values(2,:)-fraction_values(3,:))./sqrt(p_est*(1-p_est)*(1/n_samples(2)+1/n_samples(3)));
    p_val_stat_test_4_s=min(normcdf(z_values),normcdf(-z_values));
end

out_str='p_values (d vs 4)';
for i=1:length(cnts)
    out_str=[out_str ',' num2str(p_val_stat_test_d_4(i))];
end
out_str=[out_str];
fprintf(f,'%s\n',out_str);

out_str='p_values (d vs s)';
for i=1:length(cnts)
    out_str=[out_str ',' num2str(p_val_stat_test_d_s(i))];
end
out_str=[out_str];
fprintf(f,'%s\n',out_str);

out_str='p_values (4 vs s)';
for i=1:length(cnts)
    out_str=[out_str ',' num2str(p_val_stat_test_4_s(i))];
end
out_str=[out_str];
fprintf(f,'%s\n',out_str);
