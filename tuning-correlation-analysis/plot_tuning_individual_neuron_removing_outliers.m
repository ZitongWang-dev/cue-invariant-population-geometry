...
% end
% [tuning_corr_1_2,p_vals_1_2]=corr(tmp_tuning_1',tmp_tuning_2','Type','Pearson');
% % remove outlier end

tuning_geo_product=tuning_1.*tuning_2;
[max_vals,max_indices_1_2]=max(tuning_geo_product,[],2);
[tuning_corr_1_2,p_vals_1_2]=corr(tuning_1',tuning_2','Type','Pearson');

tuning_geo_product=tuning_1.*tuning_3;
[max_vals,max_indices_1_3]=max(tuning_geo_product,[],2);
[tuning_corr_1_3,p_vals_1_3]=corr(tuning_1',tuning_3','Type','Pearson');

tuning_geo_product=tuning_2.*tuning_3;
[max_vals,max_indices_2_3]=max(tuning_geo_product,[],2);
...
%%%%%%%%% Tuning & Scatter
sz=200;
for i=[1,38,45]%[163,12,177]%[1,38,45]%1:n_neuron
    curr_fig=figure();
    subplot(2,3,1)
    ...
    curr_tuning_1 = tuning_1(i,:);
    curr_tuning_2 = tuning_2(i,:);
    curr_tuning_3 = tuning_3(i,:);

    curr_tuning_geo = curr_tuning_1.*curr_tuning_2;
    [max_val,max_index]=max(curr_tuning_geo,[],2);
    tmp_tuning_1 = curr_tuning_1;
    tmp_tuning_2 = curr_tuning_2;
    tmp_tuning_1(max_index) = [];
    tmp_tuning_2(max_index) = [];
...
%    p=polyfit((tuning_1(i,:)-min(tuning_1(i,:)))./(max(tuning_1(i,:))-min(tuning_1(i,:))),(tuning_2(i,:)-min(tuning_2(i,:)))./(max(tuning_2(i,:))-min(tuning_2(i,:))),1);
    scatter((tmp_tuning_1-min(tuning_1(i,:)))./(max(tuning_1(i,:))-min(tuning_1(i,:))),(tmp_tuning_2-min(tuning_2(i,:)))./(max(tuning_2(i,:))-min(tuning_2(i,:))),sz,'.');
    p=polyfit((tmp_tuning_1-min(tuning_1(i,:)))./(max(tuning_1(i,:))-min(tuning_1(i,:))),(tmp_tuning_2-min(tuning_2(i,:)))./(max(tuning_2(i,:))-min(tuning_2(i,:))),1);
    [curr_tuning_corr, curr_p_val] = corr(tmp_tuning_1',tmp_tuning_2');
    ...
    title([upper(stim_type{1}) ' vs ' upper(stim_type{2}) newline 'corr=' num2str(curr_tuning_corr) newline 'p val=' num2str(curr_p_val)])

    curr_tuning_1 = tuning_1(i,:);
    curr_tuning_2 = tuning_2(i,:);
    curr_tuning_3 = tuning_3(i,:);
    
    curr_tuning_geo = curr_tuning_1.*curr_tuning_3;
    [max_val,max_index]=max(curr_tuning_geo,[],2);
    tmp_tuning_1 = curr_tuning_1;
    tmp_tuning_3 = curr_tuning_3;
    tmp_tuning_1(max_index) = [];
    tmp_tuning_3(max_index) = [];

    subplot(2,3,5)
    hold on
%    scatter((tuning_1(i,:)-min(tuning_1(i,:)))./(max(tuning_1(i,:))-min(tuning_1(i,:))),(tuning_2(i,:)-min(tuning_2(i,:)))./(max(tuning_2(i,:))-min(tuning_2(i,:))),sz,'.');
%    p=polyfit((tuning_1(i,:)-min(tuning_1(i,:)))./(max(tuning_1(i,:))-min(tuning_1(i,:))),(tuning_2(i,:)-min(tuning_2(i,:)))./(max(tuning_2(i,:))-min(tuning_2(i,:))),1);
    scatter((tmp_tuning_1-min(tuning_1(i,:)))./(max(tuning_1(i,:))-min(tuning_1(i,:))),(tmp_tuning_3-min(tuning_3(i,:)))./(max(tuning_3(i,:))-min(tuning_3(i,:))),sz,'.');
    p=polyfit((tmp_tuning_1-min(tuning_1(i,:)))./(max(tuning_1(i,:))-min(tuning_1(i,:))),(tmp_tuning_3-min(tuning_3(i,:)))./(max(tuning_3(i,:))-min(tuning_3(i,:))),1);
    [curr_tuning_corr, curr_p_val] = corr(tmp_tuning_1',tmp_tuning_3');
    ...
    ylim([0,1])
    %title([stim_type{1} ' vs ' stim_type{2} newline 'corr=' num2str(tuning_corr_1_2(i,i)) newline 'is\_significant=' num2str(p_vals_1_2(i,i)<0.05)])
    title([upper(stim_type{1}) ' vs ' upper(stim_type{3}) newline 'corr=' num2str(curr_tuning_corr) newline 'p val=' num2str(curr_p_val)])

    curr_tuning_1 = tuning_1(i,:);
    curr_tuning_2 = tuning_2(i,:);
    curr_tuning_3 = tuning_3(i,:);
    
    curr_tuning_geo = curr_tuning_2.*curr_tuning_3;
    [max_val,max_index]=max(curr_tuning_geo,[],2);
    tmp_tuning_2 = curr_tuning_2;
    tmp_tuning_3 = curr_tuning_3;
    tmp_tuning_2(max_index) = [];
...
%    scatter((tuning_1(i,:)-min(tuning_1(i,:)))./(max(tuning_1(i,:))-min(tuning_1(i,:))),(tuning_2(i,:)-min(tuning_2(i,:)))./(max(tuning_2(i,:))-min(tuning_2(i,:))),sz,'.');
%    p=polyfit((tuning_1(i,:)-min(tuning_1(i,:)))./(max(tuning_1(i,:))-min(tuning_1(i,:))),(tuning_2(i,:)-min(tuning_2(i,:)))./(max(tuning_2(i,:))-min(tuning_2(i,:))),1);
    scatter((tmp_tuning_2-min(tuning_2(i,:)))./(max(tuning_2(i,:))-min(tuning_2(i,:))),(tmp_tuning_3-min(tuning_3(i,:)))./(max(tuning_3(i,:))-min(tuning_3(i,:))),sz,'.');
    p=polyfit((tmp_tuning_2-min(tuning_2(i,:)))./(max(tuning_2(i,:))-min(tuning_2(i,:))),(tmp_tuning_3-min(tuning_3(i,:)))./(max(tuning_3(i,:))-min(tuning_3(i,:))),1);
    [curr_tuning_corr, curr_p_val] = corr(tmp_tuning_2',tmp_tuning_3');
    ...
    ylim([0,1])
    %title([stim_type{1} ' vs ' stim_type{2} newline 'corr=' num2str(tuning_corr_1_2(i,i)) newline 'is\_significant=' num2str(p_vals_1_2(i,i)<0.05)])
    title([upper(stim_type{2}) ' vs ' upper(stim_type{3}) newline 'corr=' num2str(curr_tuning_corr) newline 'p val=' num2str(curr_p_val)])
    ...
    print(['C:\Users\84705\Desktop\' gabor_type '_' num2str(i) '.eps'],'-depsc','-painters','-loose',gcf)
    close(curr_fig)
